...
class CachePersistenceRepositoryIntegrationTests @Autowired constructor(
    ...
    fun `findById should not return soft-deleted entity`(): Unit = runBlocking {
        ...
        val cacheId = "cache1"
        val e = KvCacheEntity(key = "k1", value = "v1",cache = cacheId, expireAt = now.plusHours(1), deletedAt = null)
        cacheRepo.save(e)
    ...
    fun `deleteById should soft-delete a row`(): Unit = runBlocking {
        val now = OffsetDateTime.now()
        val cacheId = "cache1"
        val e = KvCacheEntity(key = "k1", value = "v1",cache = cacheId, expireAt = now.plusHours(1))
        cacheRepo.save(e)
    ...
    fun `findByCacheIdExpired should return only expired non-deleted entries for the given cacheId`(): Unit = runBlocking {
        ...
        val dummyEntries = listOf(
            KvCacheEntity("expired_1", "old", cache1, now.minusHours(1)),
            KvCacheEntity("expired_2_deleted", "old_deleted", cache1, now.minusDays(1), now.minusDays(1)),
            KvCacheEntity("fresh_1", "new", cache1, now.plusHours(1))
        )
    ...
    fun `deleteExpiredByCacheId should soft delete expired rows for a given cacheId and return count`(): Unit = runBlocking {
        ...
        val cache1 = "cache1"
        val entities = listOf(
            KvCacheEntity("e1", "v", cache1, now.minusMinutes(10)),
            KvCacheEntity("f1", "v", cache1, now.plusHours(1))
        )
    ...
    fun `purgeDeleteEntries should remove rows with deleted_at not null`(): Unit = runBlocking {
        val now = OffsetDateTime.now()
        val cacheId = "cache1"
        val toPurge = KvCacheEntity("d1", "v", cacheId, now.plusHours(1), now.minusDays(1))
        val keep = KvCacheEntity("k1", "v", cacheId, now.plusHours(1), null)
    ...
    fun `findByCacheId should return all non-deleted entries for a given cacheId`(): Unit = runBlocking {
        ...
        val dummyEntries = listOf(
            KvCacheEntity("key1", "val1", cache1, now.plusHours(1)),
            KvCacheEntity("key2_deleted", "val2", cache1, now.plusHours(1), now.minusDays(1))
        )
    ...
    fun `findByCache should return all non-deleted entries for a given cache`(): Unit = runBlocking {
        val now = OffsetDateTime.now()
        val entities = listOf(
            KvCacheEntity("k1", "v1", "c1", now.plusHours(1)),
            KvCacheEntity("k2", "v2", "c2", now.plusHours(1)),
            KvCacheEntity("k3_deleted", "v3", "c1", now.plusHours(1), now.minusDays(1))
        )
        cacheRepo.saveAll(entities).toList()
    ...
    fun `upsert should insert or update a cache entry`(): Unit = runBlocking {
        ...
        // 1. Test insert
        val toInsert = KvCacheEntity(key1, "v1", cacheId, now.plusHours(1))
        ...
        // 2. Test update
        val original = KvCacheEntity(key2, "v_orig", cacheId, now.plusHours(1))
        cacheRepo.save(original)
        val toUpdate = KvCacheEntity(key2, "v_updated", cacheId, now.plusDays(1))
        cacheRepo.upsert(toUpdate)
        val updated = cacheRepo.findById(key2, cacheId)
        assertNotNull(updated)
        assertEquals("v_updated", updated.value)
        assertTrue(updated.expireAt.isAfter(now.plusHours(2)))

        // 3. Test reviving a soft-deleted entry
        val toDelete = KvCacheEntity(key3, "v_initial", cacheId, now.plusHours(1))
        cacheRepo.save(toDelete)
        cacheRepo.deleteById(key3, cacheId)
        assertNull(cacheRepo.findById(key3, cacheId))

        val toRevive = KvCacheEntity(key3, "v_revived", cacheId, now.plusDays(1))
